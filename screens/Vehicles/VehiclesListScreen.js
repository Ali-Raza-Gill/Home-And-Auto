...
//   const load = async () => {
//     setRefreshing(true);
//     const { data, error } = await supabase.from("vehicles").select("*").order("created_at", { ascending: false }).limit(50);
...
//         <Button title="Add Vehicle" onPress={() => navigation.navigate("VehicleDetail", { mode: "add" })} className="w-36 py-2" />
//       </View>
//       <View className="gap-3">
//         {vehicles.map((v) => (
//           <TouchableOpacity key={v.id} onPress={() => navigation.navigate("VehicleDetail", { id: v.id })}>
//             <Card>
//               <Text className="text-lg font-semibold">{v.nickname || `${v.year || ""} ${v.make || ""} ${v.model || ""}`}</Text>
//               <Text className="text-slate-500">{v.mileage ? `${v.mileage} miles` : ""}</Text>
//             </Card>
//           </TouchableOpacity>
//         ))}
//         {vehicles.length === 0 && <Text className="text-slate-500">No vehicles yet. Tap "Add Vehicle" to create one.</Text>}
//       </View>
//     </ScrollView>
//   );
// }
