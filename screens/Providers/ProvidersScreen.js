...
//   const load = async () => {
//     setRefreshing(true);
//     const { data, error } = await supabase.from("service_providers").select("*").order("created_at", { ascending: false }).limit(50);
...
//         <Text className="text-2xl font-bold">Service Providers</Text>
//         <Button title="Add Provider" onPress={() => navigation.navigate("AddProvider")} className="w-40 py-2" />
//       </View>
//       <View className="gap-3">
//         {providers.map((p) => (
//           <Card key={p.id}>
//             <Text className="text-lg font-semibold">{p.company_name}</Text>
//             <Text className="text-slate-500">{p.service_type} • {p.service_area}</Text>
//             {p.website && <Text className="text-haa-primary mt-1">{p.website}</Text>}
//           </Card>
//         ))}
//         {providers.length === 0 && <Text className="text-slate-500">No providers yet. Tap "Add Provider".</Text>}
//       </View>
//     </ScrollView>
//   );
// }
